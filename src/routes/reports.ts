...
router.get("/internal", authenticate, async (req: any, res) => {
  const reports = await db.prepare("SELECT * FROM support_reports WHERE type = 'internal' ORDER BY date DESC").all();
  res.json(reports);
});
...
router.get("/tasks", authenticate, async (req: any, res) => {
  const reports = await db.prepare("SELECT * FROM task_reports ORDER BY created_at DESC").all();
  res.json(reports);
});

// Create task report
router.post("/tasks", authenticate, async (req: any, res) => {
  ...
  await stmt.run(task_assign, time, result, new Date().toISOString());
  logAction('create', 'task_report', null, `Created task report`, req.user.id, req.user.name, getClientIp(req));
  res.json({ success: true });
});

// Get internal reports
router.get("/internal_reports", authenticate, async (req: any, res) => {
  const reports = await db.prepare("SELECT * FROM internal_reports ORDER BY created_at DESC").all();
  res.json(reports);
});
