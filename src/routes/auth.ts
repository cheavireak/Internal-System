...
router.post("/login", async (req, res) => {
  try {
    ...
    await db.prepare("DELETE FROM blocked_ips WHERE blocked_at <= datetime('now', '-15 minutes')").run();

    // Check if IP is blocked
    const isBlocked = await db.prepare("SELECT *, (strftime('%s', 'now') - strftime('%s', blocked_at)) as elapsed FROM blocked_ips WHERE ip = ?").get(clientIp) as any;
    if (isBlocked) {
    ...
    const handleFailedLogin = async (reason: string, userId?: number, userName?: string) => {
      ...
      await db.prepare("INSERT INTO login_attempts (ip) VALUES (?)").run(clientIp);
      
      // Clean up old attempts
      await db.prepare("DELETE FROM login_attempts WHERE timestamp < datetime('now', '-15 seconds')").run();
      
      // Check if limit reached
      const recentAttempts = await db.prepare("SELECT COUNT(*) as count FROM login_attempts WHERE ip = ? AND timestamp >= datetime('now', '-15 seconds')").get(clientIp) as {count: number};
      if (recentAttempts.count >= 5) {
        await db.prepare("INSERT INTO blocked_ips (ip, blocked_at) VALUES (?, CURRENT_TIMESTAMP) ON CONFLICT (ip) DO UPDATE SET blocked_at = CURRENT_TIMESTAMP").run(clientIp);
    ...
    await db.prepare("DELETE FROM login_attempts WHERE ip = ?").run(clientIp);
    ...
    try {
      const ipWhitelistSetting = await db.prepare("SELECT value FROM system_settings WHERE key = 'ip_whitelist'").get() as any;
